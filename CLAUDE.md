...
Minimum SDK is 21 (14+ historically per README, but `minSdk` is currently 21 in both modules); target/compile SDK is 35. Many code paths must support API 21 through 35, so SDK-version gating is a first-class concern throughout `ScreenUtility` and `ScreenCheckerActivity` — don't add calls to newer APIs without an SDK check and a sensible fallback for older versions.
